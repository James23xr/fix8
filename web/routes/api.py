...
# In-memory session store (mapping user_id to Fix8Core instance)
# This avoids pickling complex pandas/numpy structures to Postgres in development.
ACTIVE_SESSIONS = {}

def get_engine():
    """Retrieve or create the Fix8Core engine for the current authenticated user."""
    # Since we are adopting Flask-Login, use current_user.id
    # Fallback to a session UUID if not strictly logged in yet for some reason
    if current_user.is_authenticated:
        ...
        uid = f"anon_{session['uid']}"
        
    if uid not in ACTIVE_SESSIONS:
        ACTIVE_SESSIONS[uid] = Fix8Core()
        print(f"[*] Initialized new Fix8Core engine for: {uid}")
...
def api_load_demo():
    ...
    try:
        ...
        engine.eye_events = pd.DataFrame({"x_cord": x_cord, "y_cord": y_cord, "duration": duration, "eye_event": "fixation"})
        engine.trial_path = demo_path
        return jsonify({"message": "Demo data loaded", "fixation_count": len(engine.eye_events), "state": _get_state_payload(engine)})
...
def api_distort_noise():
    ...
    return jsonify({"message": f"Noise applied", "state": _get_state_payload(engine)})
